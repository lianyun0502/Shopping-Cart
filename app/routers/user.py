...
async def create_user(user: user.CreateUser=Body(...), 
    ...
    if db_session.query(Users).filter(Users.id == user.id).scalar():
        raise HTTPException(status_code=400, detail="User already registered")
    
    db_session.add(Users(**user.model_dump()))
    db_session.commit()
...
async def delete_user(user_id: str, 
    ...
    user = db_session.query(Users).filter(Users.id == user_id).scalar()
    if user is None:
